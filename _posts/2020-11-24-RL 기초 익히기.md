...
$$ \begin{aligned} \mathcal{P}^{a}_{ss’} = \mathcal{P}(S_{t+1}=s’\mid A_{t}=a, S_{t}=s) \end{aligned}$$

$\mathcal{R}$는 Reward 함수로 각 단계의 reward의 기댓값을 도출합니다. 수식으로 표현하면,  
  
$$\begin{aligned}  R^{a}_{s} &= \mathbb{E}[R_{t+1}\mid A_{t}=a, S_{t}=s]. \end{aligned}$$  
...
$$\begin{aligned} \pi (a\mid s) &=P(\mathcal{A_{t}}=a \mid \mathcal{S_{t}}=s) \end{aligned}$$  
...
$$\begin{aligned} V_{\pi}(s) &= \mathbb{E}_{\pi}(G_{t}\mid S_{t}=s)\end{aligned}$$
...
$$\begin{aligned} q_{\pi}(s,a)\ &= \mathbb{E}_{\pi}(G_{t}\mid S_{t}=s, A_{t}=a) \end{aligned}$$
...
$$\begin{aligned} \pi(a’ \mid s) &= \text{argmax}_{a’} Q(s, a’) \end{aligned}$$
...
$ Q(s,a) + \alpha (R^{a}_{s} + \gamma \text{max}_{a}Q(s’,a) - Q(s,a))$ 는 incremental mean에서 온 것으로,  
